...
typedef struct {
    ...
    // 引用计数：正在等待该条目的任务数。物理释放（free/delete sem）必须等 ref_count==0。
    // Reference count of waiters; physical free must wait until ref_count==0.
    uint8_t ref_count;
...
/**
 * @brief 物理释放一个条目占用的内存/信号量
 *        Physically release an entry's memory/semaphore
 */
static void entry_physical_free(entry_t *entry) {
 ...
 *        释放一个条目
 *
 * 先做逻辑释放（in_use=false）。若仍有等待者持有引用（ref_count>0），延迟物理释放，
 * 由等待者结束时调用 entry_consume 完成；否则立即物理释放。
 *
...
/**
 * @brief 等待者结束等待后消费条目：归还引用并标记释放。
 *        A waiter consumes the entry after finishing: drop reference and free it.
 */
static void entry_consume(entry_t *entry) {
    if (entry->ref_count > 0) {
        entry->ref_count--;
    }
    free_entry(entry);
...
static entry_t* allocate_entry_by_seq(uint16_t seq) {
    ...
    for (int i = 0; i < MAX_SEQ_ENTRIES; i++) {
        if (!s_entries[i].in_use && s_entries[i].ref_count == 0) {
        ...
        // 最久未使用的条目（跳过有等待者持有的条目）
        if (s_entries[i].ref_count == 0 && s_entries[i].last_access_time < oldest_access_time) {
...
static entry_t* allocate_entry_by_cmd(uint8_t cmd_set, uint8_t cmd_id) {
    ...
    for (int i = 0; i < MAX_SEQ_ENTRIES; i++) {
        ...
        // 仅考虑非基于 seq 的条目作为候选删除对象（跳过有等待者持有的条目）
        if (!s_entries[i].is_seq_based && s_entries[i].ref_count == 0 && s_entries[i].last_access_time < oldest_access_time) {
            oldest_access_time = s_entries[i].last_access_time;
            oldest_entry = &s_entries[i];
        }
    }
...
esp_err_t data_wait_for_result_by_cmd(uint8_t cmd_set, uint8_t cmd_id, int timeout_ms, uint16_t *out_seq, void **out_result, size_t *out_result_length) {
    ...
    while (true) {
        ...
        if (entry) {
            ...
            // 条目存在但还没有结果，需要等待
            entry->ref_count++;   // 持有引用，防止等待期间被物理释放
...
static void process_notification_data(const uint8_t *raw_data, size_t raw_data_length) {
    ...
    if (raw_data[0] == 0xAA || raw_data[0] == 0xaa) {
        ...
        // Camera-pushed status notifications (1D02/1D06) are not responses to any
        // request, so they never match a waiting seq entry. Route them straight to
        // the status callbacks below instead of the request/response entry map.
        // 相机主动推送的状态通知（1D02/1D06）不是任何请求的应答，不会命中等待中的 seq 条目，
        // 直接交给下面的状态回调处理，不再经过请求/应答条目映射。
        bool is_push = (actual_cmd_set == 0x1D && (actual_cmd_id == 0x02 || actual_cmd_id == 0x06));

        // Find corresponding entry
        // 查找对应的条目
        if (!is_push && xSemaphoreTake(s_map_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
        ...
        // Pushed notifications skip the entry map, so free the parse result here;
        // the callbacks above already made their own copies.
        // 推送通知未存入条目映射，回调已自行拷贝，这里释放解析结果避免内存泄漏。
        if (is_push) {
            free(parse_result);
        }
    } else {
