...
uint8_t current_photo_ratio = 0;
uint16_t current_real_time_countdown = 0;
uint32_t current_photo_countdown_ms = 0;
...
char current_product_id[17] = {0};
uint32_t current_camera_device_id = 0;
...
int subscript_camera_status(uint8_t push_mode, uint8_t push_freq) {
    ...
    camera_status_subscription_command_frame_t command_frame = {
        .push_mode = push_mode,
        .push_freq = push_freq,
        .reserved = {0, 0, 0, 0}
    };
...
void update_camera_state_handler(void *data) {
    ...
    const camera_status_push_command_frame_t *parsed_data = (const camera_status_push_command_frame_t *)data;
    ...
    // Check and update photo ratio
    // 检查并更新拍照比例
    if (current_photo_ratio != parsed_data->photo_ratio) {
        current_photo_ratio = parsed_data->photo_ratio;
        ESP_LOGI(TAG, "Photo ratio updated to: %d", current_photo_ratio);
        state_changed = true;
    }

    // Update photo countdown & real-time countdown (not logged: countdown ticks every second)
    // 更新拍照定时档位与实时倒计时（不打印：倒计时每秒变化，避免刷屏）
    current_photo_countdown_ms = parsed_data->photo_countdown_ms;
    current_real_time_countdown = parsed_data->real_time_countdown;

    // Update battery percentage and remaining storage (not logged to avoid spam)
    // 更新电量与剩余容量（不打印，避免刷屏）
    current_camera_bat_percentage = parsed_data->camera_bat_percentage;
...
void update_new_camera_state_handler(void *data) {
    ...
    const new_camera_status_push_command_frame_t *parsed_data = (const new_camera_status_push_command_frame_t *)data;
    ...
    // 显式 NUL 终止：按长度截断，最多保留 19 字节 + '\0'
    if (current_mode_name_length < 20) {
        current_mode_name[current_mode_name_length] = '\0';
    } else {
        current_mode_name[19] = '\0';
    }
    ...
    // 显式 NUL 终止：按长度截断，最多保留 19 字节 + '\0'
    if (current_mode_param_length < 20) {
        current_mode_param[current_mode_param_length] = '\0';
    } else {
        current_mode_param[19] = '\0';
    }
