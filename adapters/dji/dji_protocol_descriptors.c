...
#include "dji_protocol_descriptors.h"
...
const data_descriptor_t data_descriptors[] = {
    ...
    // 相机状态订阅
    {0x1D, 0x05, (data_creator_func_t)camera_status_subscription_creator, (data_parser_func_t)camera_status_subscription_parser},
    // Camera status push
    // 相机状态推送
...
uint8_t* gps_data_creator(const void *structure, size_t *data_length, uint8_t cmd_type) {
    ...
    if ((cmd_type & 0x20) == 0) {
        const gps_data_push_command_frame_t *gps_command_frame = (const gps_data_push_command_frame_t *)structure;

        *data_length = sizeof(gps_data_push_command_frame_t);

        ESP_LOGI(TAG, "Data length calculated for gps_data_push_command_frame_t: %zu", *data_length);
        ...
        const gps_data_push_response_frame_t *gps_response_frame = (const gps_data_push_response_frame_t *)structure;

        *data_length = sizeof(gps_data_push_response_frame_t);

        ESP_LOGI(TAG, "Data length calculated for gps_data_push_response_frame_t: %zu", *data_length);
...
int gps_data_parser(const uint8_t *data, size_t data_length, void *structure_out, uint8_t cmd_type) {
    ...
    if (data_length < sizeof(gps_data_push_response_frame_t)) {
        ESP_LOGE(TAG, "gps_data_parser: Data length too short. Expected: %zu, Got: %zu",
                 sizeof(gps_data_push_response_frame_t), data_length);
        return -1;
    }

    const gps_data_push_response_frame_t *response = (const gps_data_push_response_frame_t *)data;

    gps_data_push_response_frame_t *output_frame = (gps_data_push_response_frame_t *)structure_out;
    output_frame->ret_code = response->ret_code;
    return 0;
}
...
uint8_t* connection_data_creator(const void *structure, size_t *data_length, uint8_t cmd_type) {
    ...
    if ((cmd_type & 0x20) == 0) {
        ...
        *data_length = sizeof(connection_request_command_frame_t);
        ...
        ESP_LOGI(TAG, "Data length calculated for connection_request_command_frame_t: %zu", *data_length);
        ...
        *data_length = sizeof(connection_request_response_frame_t);
        ...
        ESP_LOGI(TAG, "Data length calculated for connection_request_response_frame_t: %zu", *data_length);
...
int connection_data_parser(const uint8_t *data, size_t data_length, void *structure_out, uint8_t cmd_type) {
    ...
    if ((cmd_type & 0x20) == 0) {
        ...
        if (data_length < sizeof(connection_request_command_frame_t)) {
            ESP_LOGE(TAG, "connection_request_data_parser: Data length too short for command frame. Expected: %zu, Got: %zu",
                     sizeof(connection_request_command_frame_t), data_length);
            return -1;
        }

        const connection_request_command_frame_t *command = (const connection_request_command_frame_t *)data;

        connection_request_command_frame_t *output_command = (connection_request_command_frame_t *)structure_out;
        ...
        if (data_length < sizeof(connection_request_response_frame_t)) {
            ESP_LOGE(TAG, "connection_request_data_parser: Data length too short for response frame. Expected: %zu, Got: %zu",
                     sizeof(connection_request_response_frame_t), data_length);
            return -1;
        }

        const connection_request_response_frame_t *response = (const connection_request_response_frame_t *)data;

        connection_request_response_frame_t *output_response = (connection_request_response_frame_t *)structure_out;
...
uint8_t* camera_status_subscription_creator(const void *structure, size_t *data_length, uint8_t cmd_type) {
    ...
    if ((cmd_type & 0x20) == 0) {
        const camera_status_subscription_command_frame_t *camera_command_frame = 
            (const camera_status_subscription_command_frame_t *)structure;

        *data_length = sizeof(camera_status_subscription_command_frame_t);
        
        ESP_LOGI(TAG, "Data length calculated for camera_status_subscription_command_frame_t: %zu", *data_length);
...
int camera_status_subscription_parser(const uint8_t *data, size_t data_length, void *structure_out, uint8_t cmd_type) {
    if (structure_out == NULL) {
        ESP_LOGE(TAG, "camera_status_subscription_parser: NULL structure_out detected");
        return -1;
    }

    // Only response (ACK) frames are expected for the subscription command.
    // 订阅命令只会收到应答帧（ACK）。
    if ((cmd_type & 0x20) == 0) {
        ESP_LOGW(TAG, "camera_status_subscription_parser: Parsing command frame is not supported.");
        return -1;
    }

    // The ACK may carry a ret_code; an empty payload is also a valid acknowledgement.
    // ACK 可能带 ret_code；空载荷同样视为有效应答。
    if (data_length == 0) {
        ESP_LOGI(TAG, "Camera Status Subscription ACK received (empty payload).");
        return 0;
    }

    camera_status_subscription_response_frame_t *output = (camera_status_subscription_response_frame_t *)structure_out;
    output->ret_code = data[0];

    ESP_LOGI(TAG, "Camera Status Subscription Response parsed successfully. ret_code: %u", output->ret_code);
    return 0;
}
...
int camera_status_push_data_parser(const uint8_t *data, size_t data_length, void *structure_out, uint8_t cmd_type) {
    ...
    if ((cmd_type & 0x20) == 0) {
        if (data_length < sizeof(camera_status_push_command_frame_t)) {
            ESP_LOGE(TAG, "camera_status_push_data_parser: Data length too short for command frame. Expected: %zu, Got: %zu",
                     sizeof(camera_status_push_command_frame_t), data_length);
            return -1;
        }

        const camera_status_push_command_frame_t *frame = (const camera_status_push_command_frame_t *)data;

        camera_status_push_command_frame_t *output_frame = (camera_status_push_command_frame_t *)structure_out;
...
int new_camera_status_push_data_parser(const uint8_t *data, size_t data_length, void *structure_out, uint8_t cmd_type) {
    ...
    if ((cmd_type & 0x20) == 0) {
        if (data_length < sizeof(new_camera_status_push_command_frame_t)) {
            ESP_LOGE(TAG, "new_camera_status_push_data_parser: Data length too short for command frame. Expected: %zu, Got: %zu",
                     sizeof(new_camera_status_push_command_frame_t), data_length);
            return -1;
        }

        const new_camera_status_push_command_frame_t *frame = (const new_camera_status_push_command_frame_t *)data;

        new_camera_status_push_command_frame_t *output_frame = (new_camera_status_push_command_frame_t *)structure_out;
