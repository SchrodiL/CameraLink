...
ble_profile_t s_ble_profile = {
    ...
    .remote_bda = {0},  // 上次连接设备的 MAC 地址，开机时从 NVS 载入；初始为空
...
/* ---------- 上次连接设备地址的持久化（NVS） ---------- */
#define BLE_NVS_NAMESPACE "ble_peer"
#define BLE_NVS_KEY_MAC    "peer_mac"

esp_err_t ble_save_peer_addr(const esp_bd_addr_t addr) {
    nvs_handle_t handle;
    esp_err_t err = nvs_open(BLE_NVS_NAMESPACE, NVS_READWRITE, &handle);
    if (err != ESP_OK) return err;
    err = nvs_set_blob(handle, BLE_NVS_KEY_MAC, addr, ESP_BD_ADDR_LEN);
    if (err == ESP_OK) nvs_commit(handle);
    nvs_close(handle);
    return err;
}

static esp_err_t ble_load_peer_addr(esp_bd_addr_t addr) {
    nvs_handle_t handle;
    esp_err_t err = nvs_open(BLE_NVS_NAMESPACE, NVS_READONLY, &handle);
    if (err != ESP_OK) return err;
    size_t len = ESP_BD_ADDR_LEN;
    err = nvs_get_blob(handle, BLE_NVS_KEY_MAC, addr, &len);
    nvs_close(handle);
    if (err != ESP_OK || len != ESP_BD_ADDR_LEN) return ESP_ERR_NOT_FOUND;
    return ESP_OK;
}
 ...
 */
esp_err_t ble_init() {
    /* BLE 栈已由 ble_stack_init() 统一拉起，这里只做 GATTC 主机角色初始化。 */
...
esp_err_t ble_connect_to_address(esp_bd_addr_t addr) {
    ...
    return esp_ble_gattc_open(s_ble_profile.gattc_if,
                              addr,
...
/**
 * @brief Stop the DJI client role: disconnect + stop scan + unregister GATTC app + reset state.
 * 停止 DJI 主机角色：断开 + 停扫描 + 反注册 GATTC 应用 + 复位状态。
 */
esp_err_t ble_client_stop(void) {
    ...
    if (s_ble_profile.gattc_if != ESP_GATT_IF_NONE) {
        esp_ble_gattc_app_unregister(s_ble_profile.gattc_if);
    }

    s_ble_profile.gattc_if = ESP_GATT_IF_NONE;
    s_ble_profile.conn_id = 0;
    ...
    ESP_LOGI(TAG, "ble_client_stop done");
    return ESP_OK;
}
    ...
    case ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT:
        ...
        if (best_rssi > -128) {
            if (!ble_get_reconnecting() || (ble_get_reconnecting() && s_found_previous_device)) {
                ble_connect_to_address(best_addr);
